...
CRLFI_L = ['SET', 'TAMPER','COOKIE', 'cn=', '%0A', '%0D']
UA_L = ["Nmap","nikto","wikto","sqlmap","bsqlbf","acunetix","havij","appscan","wpscan","apachebench","dirbuster","w3af","arachni","xspider","hydra","evasions","openvas"]
...
def analyse(line, tag):
    ...
    if SQLi_check(line) is True:
        tag.append('SQL Injection')
    if PT_check(line) is True:
        tag.append('Path Traversal Attack')
    if LDAPi_check(line) is True:
        tag.append('LDAP Injection')
    if CRLFi_check(line) is True:
        tag.append('CRLF Injection')
    if USER_AGENT_check(line) is True:
        tag.append('Malicious User agent')
...
def USER_AGENT_check(line):
    line = line['user_agent'].lower()
    check = False
    for col in UA_L:
        if col in line:
            check = True
